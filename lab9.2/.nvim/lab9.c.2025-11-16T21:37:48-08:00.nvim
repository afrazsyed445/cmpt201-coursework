...
port number). The client connects to IP address 127.0.0.1 and port 8000.
2. Is it UDP or TCP? How do you know?
  It is TCP. The socket() call uses SOCK_STREAM, which means TCP.
  socket(AF_INET, SOCK_STREAM, 0);
3. The client is going to send some data to the server. Where does it get this
data from? How can you tell in the code? It reads the data from standard input
the keyboard. The loop uses read(STDIN_FILENO, buf, BUF_SIZE) to get user-typed
data.
4. How does the client program end? How can you tell that in the code?
  The loop ends when the user enters EOF or a blank line (read() returns 0 or
1). Then the program closes the socket and calls exit(EXIT_SUCCESS). Therefore,
the client ends when the input loop finishes.
*/
...
void *handle_client(void *arg) {
  ...
  while ((num_read = read(cfd, buf, BUF_SIZE - 1)) > 0) {
    ...
    total_message_count++;
    int msg_no = total_message_count;
    pthread_mutex_unlock(&count_mutex);

    printf("Msg #%4d; Client ID %d: %s", msg_no, id, buf);
    fflush(stdout);
  }
